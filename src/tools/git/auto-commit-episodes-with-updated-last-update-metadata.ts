...
export async function autoCommitEpisodesWithUpdatedMetadata(): Promise<void> {
  ...
  for (let index = 0; index < unstagedFiles.length; index++) {
    ...
    if (unstagedFile.includes('src/markdown/')) {
      ...
      const commitMessage = `feat(${reign}): update lastUpdate metadata in episode n° ${episodeNumber}`;
...
autoCommitEpisodesWithUpdatedMetadata().then();
