...
export async function autoCommitFormattedFrenchEpisodes(): Promise<void> {
  ...
  for (let index = 0; index < unstagedFiles.length; index++) {
    ...
    if (unstagedFile.includes('src/markdown/fr/')) {
      ...
      const commitMessage = `feat(${reign}/fr): format episode n° ${episodeNumber}`;
...
autoCommitFormattedFrenchEpisodes().then();
