...
async function autoCommitEpisodesWithUpdatedGeoData() {
  ...
  for (let index = 0; index < unstagedFiles.length; index++) {
    ...
    if (unstagedFile.includes('src/markdown/fr/')) {
      ...
      const commitMessage = `feat(${reign}/fr): update geodata in episode n° ${episodeNumber}`;
...
autoCommitEpisodesWithUpdatedGeoData().then();
