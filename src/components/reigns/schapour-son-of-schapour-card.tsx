...
export const SchapourSonOfSchapourCard: React.FC = () => {
  const data = useStaticQuery<AllMarkdownRemarkResponse>(graphql`
    {
      allMarkdownRemark(
        limit: 200
        filter: { fileAbsolutePath: { glob: "**/markdown/32-schapour-fils-de-schapour/**/*.md" } }
      ) {
        totalCount
        edges {
          node {
  ...
  return (
    <Link
      className="nav-link"
      to="/regne-de-schapour-fils-de-schapour/"
      aria-label="Règne de Schapour fils de Schapour"
    >
      <div className="card text-center bg-dark text-white border-secondary">
        <div className="card-header pb-0">
          <h5 className="card-title">Schapour fils de Schapour</h5>
        </div>
