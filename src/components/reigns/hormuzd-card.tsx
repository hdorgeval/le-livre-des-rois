...
export const HormuzdCard: React.FC = () => {
  const data = useStaticQuery<AllMarkdownRemarkResponse>(graphql`
    {
      allMarkdownRemark(
        limit: 200
        filter: { fileAbsolutePath: { glob: "**/markdown/42-hormuzd/**/*.md" } }
      ) {
        totalCount
      }
    }
  ...
  return (
    <Link className="nav-link" to="/regne-de-hormuzd/" aria-label="Règne de Hormuzd">
      <div className="card text-center bg-dark text-white border-secondary">
        <div className="card-header pb-0">
          <h5 className="card-title">Hormuzd</h5>
        </div>
        <div className="card-body card-text">
          <blockquote className="blockquote mb-0">
            <p className="text-secondary">Son règne dura 14 ans.</p>
          </blockquote>
        </div>
